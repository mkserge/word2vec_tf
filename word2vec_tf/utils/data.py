from word2vec_tf.utils import checks
import collections
import numpy as np
import math
import time
...
def get_sentences_from_file(train_file):
    ...
    :param train_file:  A path to a file
    :return:            A list of sentences as they appear in the input.
    """
...
def get_words_from_file(train_file):
    ...
    :param train_file:   A path to a file
    :return:             A list of words as they appear in the input.
    """
    words = []
...
def save_word_counts(word_count, dict_file):
    # Save the dictionary into a file. Note that word_count and dictionary
    # have the same ordering except that dictionary has extra 'PAD' symbol at index 0
...
def get_data(sentences, num_total_words, dictionaries, args):
    ...
    # Ignore this as this is for me to debug stuff.
...
def downsample_sentence(sentence_in, dictionaries):
    """ Down-sample the training sentences exactly as in word2vec.
        * Words not in the vocabulary are omitted.
        * EOS symbols are also omitted.

    :param sentence_in: The input sentence that will be downsampled
    :param dictionary:  Dictionary mapping words to indices
    :param keep_prob:   Dictionary mapping words to the probability of keeping them.

    :return:            The downsampled sentence
    """
...
def get_train_data_with_sentence_downsampling(sentences, dictionaries, args):
    ...
    dictionary           -- a dictionary of the vocabulary words mapping words to indices
    reversed_dictionary  -- a dictionary mapping indices to their corresponding words
